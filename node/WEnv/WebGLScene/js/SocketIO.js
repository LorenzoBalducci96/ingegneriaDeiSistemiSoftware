...
export default (onKeyUp, onKeyDown) => {
	...
	socket.on( 'action', action => makeAction(action) )	
	...
	eventBus.subscribe(eventBusEvents.movement_succeeded, movement => {
		socket.emit('movement_succeeded', movement);
	})
	eventBus.subscribe(eventBusEvents.action, action => {
		alert("ricevuta action");
		socket.emit('action', movement);
	})
    const keycodes = {
        W: 87,
        A: 65,
