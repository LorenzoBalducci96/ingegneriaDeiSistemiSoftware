...
function WebpageServer(callbacks) {
	...
	this.action = action => Object.keys(sockets).forEach( key => sockets[key].emit('action', action) ),
...
function initSocketIOServer(callbacks) {
    socketIO.on('connection', socket => {
		...
		socket.on( 'action', callbacks.onAction )
        socket.on( 'sonarActivated', callbacks.onSonarActivated )
        socket.on( 'collision', callbacks.onCollision )
		socket.on( 'movement_succeeded', callbacks.onMovement_succeeded )
